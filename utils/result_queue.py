...
class FrameQueue():
    def __init__(self, max_length=30, det_batch_size=1, lmk_batch_size=1, rec_batch_size=1):
        ...
        self.address_list = []
        self.box_start = {"frame_index": 0, "face_index": 0}
        self.box_lock = threading.Lock()
        self.kpt_start = {"frame_index": 0, "face_index": 0}
        self.kpt_lock = threading.Lock()
    ...
    def put_frame(self, frame_id, img):
        ...
        # 阻塞加入，当空间不够时等待执行 self.get_result() 释放空间后再加入
    ...
    def get_batch_boxes(self):
        ...
        with self.box_lock:
            face_info = []
            boxes = []
            frame_idx, face_idx = self.box_start
